...
public class LocalDeviceName {
    private static Boolean MYLOG_SWITCH = true; // 日志文件总开关
    private static Boolean MYLOG_WRITE_TO_FILE = true;// 日志写入文件开关
    private static char MYLOG_TYPE = 'v';// 输入日志类型，w代表只输出告警信息等，v代表输出所有信息
    private static String MYLOG_PATH_SDCARD_DIR = "/sdcard/LocalAppLogs/log/Device";// 日志文件在sdcard中的路径
    private static int SDCARD_LOG_FILE_SAVE_DAYS = 1;// sd卡中日志文件的最多保存天数
    private static String MYLOGFILEName = "Device.log";// 本类输出的日志文件名称
    ...
    public static  void DeleteFileNow(){
        new Thread(new Runnable() {
            @Override
            public void run() {
                File dirPath = Environment.getExternalStorageDirectory();
